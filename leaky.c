...
if(sent < outgoing && store != 0) //if incoming<outgoing, we can add values from the store to besent till op cap is reached
{
int remaining = outgoing-sent;
while(remaining > 0 && store != 0) //keeps adding one to sent until we run out of op cap ornothing left in buff
{
remaining -= 1;
store -= 1;
sent += 1;
...
{
int dropped = (incoming-outgoing)-(buck_capacity-store); //excess packets are the ones that aftersending cant be accomodated in buff
